...
// NOTE: Do NOT default to a local MongoDB instance. Require explicit MONGODB_URI to enable Mongo.
const MONGO_URI = process.env.MONGODB_URI || null;

if (!process.env.MONGODB_URI) {
  console.warn('⚠️ MONGODB_URI is not set — MongoDB is disabled. The app will use filesystem fallback by default.');
}
...
async function connect() {
  if (!MONGO_URI) {
    throw new Error('MONGODB_URI is not set — MongoDB disabled. Set MONGODB_URI to enable MongoDB.');
  }
